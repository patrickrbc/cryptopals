...
function divideInBlocks (string, size) {
  ...
  for (let i = 0, len = Math.floor(string.length / size); i < len; i++)
    blocks.push(string.slice(i * size, (i * size) + size))
  return blocks
}
...
/**
* Count the number of repetitions of a string in an array of strings
* @param {String} string
* @param {String[]} array of strings
* @return {Number} number of repetitions
*/
function countRepetitions (string, arr) {
  var sum = 0
  arr.map((x, i) => (x === string) ? sum++ : 0)
  return sum - 1
}
